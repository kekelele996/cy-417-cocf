...
TripWeaver 是一款纯前端旅行规划应用，支持创建旅行、探索景点、编排每日行程、同行人分工、预算统计和分享预览。

## 主要功能

- 我的旅行：创建、筛选、删除旅行计划。
- 行程详情：查看每日行程、预算图表、共享时间线与同行人管理。
- 景点探索：按 SpotCategory 搜索和筛选，收藏并加入行程。
- 行程编排：SortableJS 拖拽排序，实时影响预算计算；为每个日程项指定 1 名负责人和多名协助人。
- 同行人分工：同一负责人同一天时间重叠的日程项最多 1 项（协助不受限）；移除同行人时其负责项一次性整批转交，任何一项找不到无冲突接收人则整批拒绝，成员与分工保持原样。
- 分享预览：生成含负责人/协助人信息、可复制的行程文本。
